...
class ProfileReporter {
  ...
  start() {
    ...
    if (self.agent.getOption('autoProfiling')) {
      if (!self.agent.config.reportOnly) {
  ...
  profile(rateLimit) {
    ...
    if (rateLimit && self.spanCount > self.config.maxSpanCount) {
  ...
  report() {
    ...
    profileData.forEach((d) => {
      ...
      metric.createMeasurement(Metric.c.TRIGGER_TIMER, d.profile.measurement, d.unitInterval, d.profile);
