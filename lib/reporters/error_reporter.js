...
class ErrorReporter {
  ...
  constructor(agent) {
    ...
    self.REGISTRATION_DELAY = 2 * 1000;
    self.FRAME_REGEXP = /^\s+at\s+(.*)$/;
  ...
  reset() {
    ...
    self.exceptionProfile = new Breakdown(self.agent, 'Error call graph', Breakdown.c.TYPE_CALLGRAPH);
    self.rejectionProfile = new Breakdown(self.agent, 'Error call graph', Breakdown.c.TYPE_CALLGRAPH);
  }
  ...
  updateProfile(profile, err) {
    ...
    frames.reverse().forEach((frame) => {
      ...
      currentNode.setType(Breakdown.c.TYPE_CALLSITE);
      currentNode.increment(1, 0);
    });
    ...
    if (!messageNode) {
      if (currentNode.children.size < 5) {
        ...
        messageNode.setType(Breakdown.c.TYPE_ERROR);
      }
      else {
  ...
  extractFrames(err) {
    ...
    lines.forEach(function(line) {
      let m = self.FRAME_REGEXP.exec(line);
      if (m && m.length == 2) {
        frames.push(m[1]);
      }
    });
